...
> 本仓库只分发**连接层**：① MaiBot 统一连接插件（hub）与 ③ 本地连接框架。
> 原先随仓库分发的业务插件（② `cateye_maibot-deepseek-harness-connect` MaiBot 端、
> ④ `cateye_connect_all-in-one_client/plugins/cateye_deepseek_harness` 本地端）
> 已从本仓库移除；如需纵深业务能力，请按下文「预留 API」自行接入或另行获取。
...
QQ ⇄ MaiBot
  ...
  └─ WS（协议 v2，见 docs/protocol-v2.md）
       ⬑ ③ cateye_connect_all-in-one_client（本地框架）
            ...
            ⬑ plugins/<你的插件>（本地业务插件，自行开发，见 docs/dev-local-plugin.md）
```
...
1. 把 ① 复制到 MaiBot 的插件目录（旧插件 `maibot-community.deepseek-harness-connect`
...
2. 浏览器打开 WebUI `http://127.0.0.1:8766`（密码见 `config.json` 的
   ...
   保存后自动重连。
3. 自建业务插件放入 `cateye_connect_all-in-one_client/plugins/<你的插件>/`，
   框架启动时自动加载（握手时上报命令注册表，hub 据此校验命令）。
...
> 截图命令由 ① 统一连接插件提供。其余业务命令（对话 / 持久 CMD / 运行 / 预设等）
> 由各自的业务插件自行注册，本仓库不再自带。
...
> `dev_tests/` 中另有若干针对本机完整工作区的用例（`smoke_test_maibot_plugins.py`
> 等），它们假定工作区中存在本仓库已不再分发的业务插件；仅 `mock_hub.py` 这条联调
> 流程可在本仓库内独立跑通。
...
├── .gitattributes                              行尾约定（.cmd/.bat 强制 CRLF）
├── LICENSE                                     MIT 许可证
├── README.md                                   本文件
├── docs/                                       协议与开发指南
├── dev_tests/                                  mock hub 与冒烟测试
├── cateye_connect_all-in-one_on_maibot/        ① 统一连接插件
└── cateye_connect_all-in-one_client/           ③ 本地框架（plugins/ 放自建插件）
```

## 许可证

本项目（①③ 两个连接层组件）以 [MIT](LICENSE) 协议发布。
